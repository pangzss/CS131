...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            X_class_i = X[idxs]
            X_centered = X_class_i - X_class_i.mean(axis = 0, keepdims = 0)
            scatter_within += np.dot(X_centered.T,X_centered)
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        mu = X.mean(axis=0,keepdims = 0)
        for i in np.unique(y):
            # YOUR CODE HERE
            idxs = np.flatnonzero(y == i)
            X_class_i = X[idxs]
            mu_i = X_class_i.mean(axis = 0, keepdims = 0)
            Ni = len(idxs)
            scatter_between += Ni*np.dot((mu_i-mu).T,mu_i-mu)
            # END YOUR CODE
