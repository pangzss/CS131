...
def compute_distances(X1, X2):
    ...
    dists = -2 * np.dot(X1, X2.T) + np.sum(X1**2,axis=1,keepdims=True) + np.sum(X2**2, axis=1,keepdims=True).T
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        idxs_k_nearest = np.argsort(dists[i,:])[:k]
        closest_y = list(y_train[idxs_k_nearest])
...
def split_folds(X_train, y_train, num_folds):
    ...
    returns:
        X_trains: numpy array o`f shape (num_folds, train_size * (num_folds-1) / num_folds, D)
    ...
    for i in range(num_folds):
        sub_X   = np.array_split(X_train,[validation_size*i,validation_size*i+validation_size])
        sub_y   = np.array_split(y_train,[validation_size*i,validation_size*i+validation_size])
        val_X   = sub_X[1]
        val_y   = sub_y[1]
        train_X = np.concatenate((sub_X[0], sub_X[2]),axis = 0)
        train_y = np.concatenate((sub_y[0], sub_y[2]))
